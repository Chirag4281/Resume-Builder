...
public class ResumeUI extends JFrame implements ActionListener {
	...
	public ResumeUI() {
		...
		t5 = new JTextField();
		t6 = new JTextField();
                im1=new JButton("Upload Photo");
                im1.setForeground(Color.white); //sets font color
		im1.setBackground(new Color(0X000075));
   ...
   im1.addActionListener(new ActionListener() {
        ...
        int option = fileChooser.showOpenDialog(ResumeUI.this);
        if (option == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            ImagePath = file.getAbsolutePath();
            b1.setEnabled(true);
            
            // Show a dialog confirming successful photo upload
            JOptionPane.showMessageDialog(ResumeUI.this, "Photo uploaded successfully", "Success", JOptionPane.INFORMATION_MESSAGE);
        }
                    // Optionally, you can display the selected image path somewhere
                    // e.g., display it in a label or text field
